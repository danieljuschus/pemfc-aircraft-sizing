# Sizing script for a fuel cell system, used by the Class2WeightEstimation module of the Initiator. The Initiator
...
# HydrogenPropulsionType == "FC"") to replace the GT/TurboGen of a serial hybrid configuration with a fuel cell system.
#
# Due to the way that the hybrid code works, the TurboGen engine (fuel cell system in this case) and the ElectroProp (EM1
...
# - One FC system consists either of one stack and its BOP, or multiple stacks in series and ONE set of BOP components -
#   the latter is useful to simplify the structural design of the stacks, only the effect on the weight is taken into
#   account
# - One FC engine consists of a FC system and the electric motor and propeller - system and engine are sometimes used
...
#
# Inputs from Matlab:
# 1: power per FC system in W - this is the "propulsive" power output that the Initiator requires from the FC system (the
#                             stacks need to produce more to also run the BOP)
# 2: number of engines (i.e. number of FC systems in parallel)
# 3: cruise altitude in m
# 4: cruise mach number
#
# Outputs:
# 1: mass of FC system in kg
# 2: cell efficiency
...
# Inputs from Matlab
power_fc_sys = float(sys.argv[1])  # power output of FC system in W
...
n_cells = volt_req/volt_cell  # number of cells in FC system
...
# Sum up to find size of FC system
m_sys = m_stack*n_stacks_series + m_comp + m_humid + m_hx
...
savemat("External/FuelCell/output_from_python.mat", {"engineMass": m_sys, "engineEfficiency": eta_cell})
#with open("output_to_matlab.txt", "w") as file:
#    file.write(str(m_sys) + "\n" + str(eta_cell))
